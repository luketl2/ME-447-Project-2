...
class cosserat_rod:
    # initialize the rod
    def __init__(self, r_o, Q_o, v_o, w_o, n_elements, rad, rho, E, G, lock_e: bool, milestone, ext_forces, ext_couples):
        self.tracker = 0
    ...
    def Newton2(self, x, Q):        
        ...
        # add damping
        nu = 0.3
        elem_vels = (self.v[:, 1:] + self.v[:, :-1])/2
        damping = -elem_vels*L*nu
        damping = self.deltaH_operator(damping)
        ...
        rhs = bend_couple + twist_couple + shear_stretch_couple + self.ext_couples
        dwdt = rhs / (self.J_ref / e)
