...
#     foot_angle = -np.pi/4
    ...
    leg_sim.collect_diagnostics(torso).using(
        RodCallback, step_skip=100, callback_params=pp_list_torso
    )
    leg_sim.collect_diagnostics(straight_leg).using(
        RodCallback, step_skip=100, callback_params=pp_list_straight_leg
    )
    leg_sim.collect_diagnostics(foot).using(
        RodCallback, step_skip=100, callback_params=pp_list_foot
    )
    leg_sim.finalize()
